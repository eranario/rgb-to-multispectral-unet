...
class PotatoDataset(Dataset):
    ...
    def __init__(self, rgb_dir, spectral_dir, transform=None, mode='train'):
        ...
        args_list = [
            (rgb_dir, spectral_dir, folder_set, rgb_name, idx, self.channels, self.spectral_files, self.align_images)
    ...
    def __getitem__(self, idx):
        rgb_image, aligned_spectral = self.data[idx]

        # convert to PIL
        rgb_image = Image.fromarray(rgb_image).convert('RGB')
        aligned_spectral = [Image.fromarray(img) for img in aligned_spectral]

        # apply transformations
        if self.transform:
            rgb_image = self.transform(rgb_image)
            aligned_spectral = [self.transform(img) for img in aligned_spectral]

        return (rgb_image, *aligned_spectral)
    ...
    def process_image(args):
        ...
        """
        rgb_dir, spectral_dir, folder_set, rgb_name, idx, channels, spectral_files, align_images = args
        ...
        for channel in channels:
            ...
            aligned_image = align_images(rgb_gray, spectral_im)
            aligned_spectral.append(aligned_image)

        return (rgb_resized, aligned_spectral)
